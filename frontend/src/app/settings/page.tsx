...
export default function SettingsPage() {
  ...
  const content = useMemo(
    () =>
      role === "creator"
        ? {
            profileHint: "This profile appears publicly to fans and potential subscribers.",
            walletLabel: "Creator payout wallet",
            walletAddress: "0xA47bF8934d6a79c161c29d98392B2f2217c2f107",
            walletNote: "Payouts are sent to this wallet after each settlement cycle.",
            notifications: ["New subscriber alerts", "Comment and message activity", "Weekly payout summaries"],
            deletionCopy:
              "Deleting a creator account removes profile data, subscription history visibility, and unreleased content drafts.",
          }
        : {
            profileHint: "This profile is visible when creators view your engagement and support activity.",
            walletLabel: "Fan payment wallet",
            walletAddress: "0xB06cb7A62Bd658fb6312F6d8Ab2dA7b778bc67c0",
            walletNote: "This wallet is used for subscription renewals and one-time support payments.",
            notifications: ["Creator post updates", "Renewal reminders", "Payment receipts"],
            deletionCopy:
  ...
  const renderSection = () => {
    ...
    if (activeSectionId === "profile") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Profile</h2>
          ...
          <div className="mt-4 flex flex-col gap-3 sm:grid sm:grid-cols-2">
            <label className="flex flex-col gap-1.5 text-sm text-slate-700">
              ...
              <input
                ...
                defaultValue={role === "creator" ? "@star.creator" : "@fan.nova"}
                type="text"
              />
            </label>
            ...
            <label className="flex flex-col gap-1.5 text-sm text-slate-700">
              ...
              <input
                className="w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-slate-300"
                defaultValue={role === "creator" ? "creator@myfans.app" : "fan@myfans.app"}
                type="email"
              />
            </label>
    ...
    if (activeSectionId === "wallet") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Wallet</h2>
          <p className="mt-1 text-sm text-slate-600">{content.walletNote}</p>

          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
            <p className="text-xs uppercase tracking-[0.12em] text-slate-500">{content.walletLabel}</p>
            {/* break-all forces the long hex string to wrap instead of overflow */}
            <p className="mt-1 break-all font-mono text-sm text-slate-800">{content.walletAddress}</p>
            <button
      ...
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Choose which updates you receive in email, push, and in-app channels.
          </p>
          ...
          <div className="mt-4 space-y-3">
            {content.notifications.map((item) => (
              ...
              >
                <span>{item}</span>
                <input className="h-4 w-4 shrink-0 accent-slate-900" defaultChecked type="checkbox" />
              </label>
            ))}
          </div>
        </section>
    ...
    if (activeSectionId === "account") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Account</h2>
          <p className="mt-1 text-sm text-slate-600">Security, access, and account lifecycle controls.</p>
    ...
    if (role === "creator" && activeSectionId === "payout") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Payout Settings</h2>
          <p className="mt-1 text-sm text-slate-600">Control how and when earnings are settled.</p>
    ...
    if (role === "creator" && activeSectionId === "content") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Content Preferences</h2>
          <p className="mt-1 text-sm text-slate-600">Set monetization defaults for future posts.</p>
    ...
    if (role === "creator" && activeSectionId === "visibility") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Creator Visibility</h2>
          <p className="mt-1 text-sm text-slate-600">Manage discoverability and public profile visibility.</p>
          <div className="mt-4 rounded-xl border border-slate-200 p-3 text-sm text-slate-700">
    ...
    if (role === "fan" && activeSectionId === "subscriptions") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Subscription Management</h2>
          <p className="mt-1 text-sm text-slate-600">Manage your active plans and renewal preferences.</p>
          <div className="mt-4 rounded-xl border border-slate-200 p-3 text-sm text-slate-700">
    ...
    if (role === "fan" && activeSectionId === "followed") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Followed Creators</h2>
          <p className="mt-1 text-sm text-slate-600">Your followed list and notification priorities.</p>
          <div className="mt-4 rounded-xl border border-slate-200 p-3 text-sm text-slate-700">
    ...
    if (role === "fan" && activeSectionId === "spending") {
      return (
        <section className="w-full rounded-2xl border border-slate-200 p-4 sm:p-5">
          <h2 className="text-base font-semibold text-slate-900 sm:text-lg">Spending History</h2>
          <p className="mt-1 text-sm text-slate-600">Review recent support and subscription activity.</p>
          <div className="mt-4 rounded-xl border border-slate-200 p-3 text-sm text-slate-700">
            ...
            {!deleteComplete ? (
              <>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  Type <strong>DELETE</strong> exactly to continue. This action is permanent.
                </p>
