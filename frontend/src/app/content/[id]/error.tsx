...
export default function ContentErrorPage({ error, reset }: ContentErrorPageProps) {
  useEffect(() => {
    logger.logError({
      message: error.message || 'Content page error',
      correlationId: error.digest,
      context: { digest: error.digest, segment: 'content' },
      error,
    });
  }, [error]);
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center p-8 text-center" role="alert">
      ...
      <h2 className="mb-2 text-xl font-semibold text-gray-900 dark:text-white">Could not load content</h2>
