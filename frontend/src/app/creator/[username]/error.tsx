...
interface CreatorErrorPageProps {
...
export default function CreatorErrorPage({ error, reset }: CreatorErrorPageProps) {
  useEffect(() => {
    logger.logError({
      message: error.message || 'Creator page error',
      correlationId: error.digest,
      context: { digest: error.digest, segment: 'creator' },
      error,
    });
  }, [error]);
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center p-8 text-center" role="alert">
      ...
      <h2 className="mb-2 text-xl font-semibold text-gray-900 dark:text-white">Could not load creator</h2>
      <p className="mb-6 max-w-sm text-sm text-gray-500 dark:text-gray-400">
        Something went wrong loading this creator&apos;s page. Please try again.
      </p>
      {error.digest && (
