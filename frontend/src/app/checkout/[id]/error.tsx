...
export default function CheckoutErrorPage({ error, reset }: CheckoutErrorPageProps) {
  useEffect(() => {
    logger.logError({
      message: error.message || 'Checkout error',
      correlationId: error.digest,
      context: { digest: error.digest, segment: 'checkout' },
      error,
    });
  }, [error]);
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center p-8 text-center" role="alert">
      ...
      <h2 className="mb-2 text-xl font-semibold text-gray-900 dark:text-white">Checkout error</h2>
