import { DashboardSectionBoundary } from '@/components/dashboard';

export default function SettingsPage() {
  return (
    <div className="max-w-full overflow-x-hidden">
      <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Settings</h1>
      <DashboardSectionBoundary label="Settings">
        <div className="bg-white dark:bg-gray-900 p-4 sm:p-6 rounded-lg border border-gray-200 dark:border-gray-800">
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">Configure your creator profile and preferences.</p>
        </div>
      </DashboardSectionBoundary>
    </div>
  );
}
